<script>
    ...
    const deposit = async() => {
        try {
            ...
            if (depositInWei > allowance)  {
                const { hash } = await masterChef.deposit(MASTERCHEF_ADDRESS, depositInWei, ZERO_ADDRESS);
                await waitForTransaction({hash});
    ...
    const refreshData = async() => {
        ...
        const totalStaked = await erc20.balanceOf(FARM_TOKEN_ADDRESS, MASTERCHEF_ADDRESS);
        ...
        if(info.type == Types.Farm) {
            ...
            tvl = await priceHelper.calculatePairValue(FARM_TOKEN_ADDRESS, totalStaked, info.oracle);
            ...
            stakingTokenPrice = await priceHelper.tokenPrice(FARM_TOKEN_ADDRESS, info.oracle);
        ...
        if ($connected) {
            ...
            if(info.type == Types.Farm) {
                ...
                stakedWPLS = await priceHelper.calculatePairValue(FARM_TOKEN_ADDRESS, staked, info.oracle);
                try {
...
<div class="border-4 border-[#00BF63] w-full bg-[#000000a3] h-fit rounded-lg">
    ...
    <div transition:slide={{ delay: 250, duration: 300, easing: quintOut, axis: 'y' }} class="p-2 space-y-6 mt-4" style="transition: none;">
        <div class="space-y-2">
            ...
            <h5>
                {#if rewards => 0}
                    {format.wei(rewards)} <span class="text-xs">Bananas</span>
                    {#if rewardsAsUSD}
